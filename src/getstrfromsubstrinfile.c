 ...
 * uasge:
    const char *fullPathToConf = "/home/diogenes/.config/diowmenu/icons.cache";
...
char *find_substring_in_file(const char *fullPathToConf, const char *substring) {
	...
	char noIconBuff[strlen(HOME) + 100];
	snprintf(noIconBuff, sizeof(noIconBuff), "%s/.config/diowmenu/noicon.svg", HOME);
